...
export const AdminFinanceiro: React.FC = () => {
  ...
  const [selectedInvoice, setSelectedInvoice] = useState<FinanceApi | null>(null);
  const [invoices, setInvoices] = useState<FinanceApi[]>([]);
  ...
  const load = () =>
    apiFetch<FinanceApi[]>('finance')
      .then(setInvoices)
      .catch((e) => setError(e instanceof ApiError ? e.message : 'Erro ao carregar financeiro.'));

  useEffect(() => {
    load();
  }, []);

  const handleOpenModal = (invoice: FinanceApi) => {
  ...
  const handlePay = async (e: React.FormEvent) => {
    ...
    try {
      await apiFetch(`finance/${selectedInvoice.id}/pay`, {
        method: 'PUT',
        body: JSON.stringify({
          paymentDate: formData.paymentDate,
          paymentMethod: formData.paymentMethod,
          generateNext: formData.generateNext,
        }),
      });
      setShowModal(false);
      ...
      alert(err instanceof ApiError ? err.message : 'Erro ao dar baixa.');
    }
  };
              ...
              <tbody className="divide-y divide-slate-50">
                {invoices.map((inv) => (
                  <tr key={inv.id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="px-6 py-4 text-sm font-semibold text-slate-700">{fmtDate(inv.dueDate)}</td>
                    <td className="px-4 py-4">
                      <p className="text-sm font-medium text-slate-900">{inv.user?.name || '—'}</p>
                    </td>
                    <td className="px-4 py-4 text-sm font-mono text-slate-800">{inv.value.toFixed(2)}</td>
                    <td className="px-4 py-4">
                      <Badge
                    ...
                    <td className="px-4 py-4 text-right">
                      {inv.status !== 'PAID' && (
                        <Button size="sm" onClick={() => handleOpenModal(inv)}>
                          Dar Baixa
                        </Button>
                      )}
                    </td>
              ...
              <form onSubmit={handlePay} className="space-y-4">
                ...
                <label className="flex items-start gap-3 p-3 rounded-xl border border-slate-200 bg-white cursor-pointer hover:bg-slate-50 transition-colors mt-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500">Cria cobrança para o mês seguinte com o mesmo valor.</p>
                  </div>
                </label>
